...
static UINT8 joypad_prev_state; /* Previous frame's joypad button state.  */
static UINT8 joypad_curr_state; /* Current frame's joypad button state.   */
...
static BOOLEAN joypad_waittime (UINT8 buttons, UINT8 timeout)
{
    for (UINT8 i = 0; i < timeout; ++i) {
        UPDATE_JOYPAD_STATE;
        if (JOYPAD_PRESSED(buttons)) {
            return TRUE;
        }
